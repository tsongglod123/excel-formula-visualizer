...
**Simple-call simplification (office-worker UX):** To keep dense formulas compact and scannable, `FormulaOutline` renders any subtree as a single inline pill (`CompactSubtree`) when it contains **no function call nested inside another function call** — `isCompactSubtree()` + `containsFunctionCall()`. So `SUM(A1:A10)` and flat fragments like `amt > avg * 1.5` stay one pill; only genuinely nested calls (`LET(…, IF(AND(…), …))`) break out into structural rows. Structural rows carry collapse toggles (chevron) and evaluation-step badges; the toolbar has **Expand all / Collapse all** bulk buttons (hidden when the formula has no collapsible groups), and collapse state lives in `CollapseContext` as a `Set<string>` (reset on new AST). Compact pills still support hover-highlight, reference-click (`role="button"`), doc popovers, and current-step rings. The outline container scrolls horizontally (`overflow-x-auto`) and the EvaluatorBar formula display caps at `max-h-40 overflow-y-auto`.
